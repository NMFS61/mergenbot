...
export default function SuccesFailTableGroupsYearByYear(props:IProps) {
    ...
    const firstDate=props.results[0].execResults[0].position.openDate;
    const len=props.results[0].execResults.length
    const lastDate=props.results[0].execResults[len-1].position.openDate
    let i=firstDate.getFullYear();
    while(i<=lastDate.getFullYear()){
      props.results.forEach((r)=>{
        const rThisYear=r.execResults.filter((er)=>er.position.openDate.getFullYear()==i);
