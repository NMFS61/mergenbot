...
function calcFailsAvgReturnfor100(fails:PositionResult[]){
    const totalSpent=fails.length*100;
    let totalWin=0;
    fails.forEach((f)=>{
        const diff=Math.abs(f.positionClose.close - f.position.close); 
        const diffPCLoss=100*diff/f.position.close;
        totalWin=100*fails.length*(1-diffPCLoss/100);
    });
    return 100*totalWin/totalSpent;
}

function avgDaysToClose(success:PositionResult[]){
    let totalDays=0;
    success.forEach((s)=>{
...
export default function SuccesFailTableGroupsYearByYear(props:IProps) {

    const _rows = [];
    const targetPC=props.results[0].execResults[0].meta.targetPercent;
    const firstDate=props.results[0].execResults[0].position.date;
    const len=props.results[0].execResults.length
    const lastDate=props.results[0].execResults[len-1].position.date
    let i=firstDate.getFullYear();
    while(i<=lastDate.getFullYear()){
      props.results.forEach((r)=>{
        const rThisYear=r.execResults.filter((er)=>er.position.date.getFullYear()==i);
    ...
    props.results.forEach((r)=>{
        ...
        _rows.push(createData(r.group,success,fail,targetPC,lossIncomefor100,avgDays));
      });
